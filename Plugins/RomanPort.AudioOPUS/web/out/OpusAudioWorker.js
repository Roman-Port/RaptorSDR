...
class OpusProcessor extends AudioWorkletProcessor {
    ...
    constructor() {
        ...
        //Subscribe to the init message
        this.port.onmessage = (evt) => this.OnCmd(evt.data);
    }
    ...
    process(inputs, outputs, parameters) {
        ...
        if (this.sync) {
            ...
            for (var i = 0; i < o[0].length; i += 1) {
                //Loop through channels
                for (var c = 0; c < o.length; c++) {
                    //Transfer
                    o[c][i] = this.ReadBufferSample() * this.volume;
                }
            }

            //Check if we should remain sync'd
            if (this.bufferAvailable <= 0) {
                this.Log("Lost synchronization. Waiting for more samples...");
                this.sync = false;
            }
        } else {
            //Clear all samples
    ...
    ReadBufferSample() {
        if (this.bufferAvailable <= 0) { return 0; }
        var value = this.buffer[this.bufferRead];
        this.bufferRead = (this.bufferRead + 1) % this.BUFFER_SIZE;
        this.bufferAvailable -= 1;
        return value;
    }

    AddBufferSamples(samples) {
        //Get how many we can write
        var writable = Math.min(samples.length, this.BUFFER_SIZE - this.bufferAvailable);
        if (writable < samples.length) {
            this.Log("Too many samples! Dropping " + (samples.length - writable) + " incoming samples...");
        }

        //Transfer
        for (var i = 0; i < writable; i += 1) {
            this.buffer[this.bufferWrite] = samples[i];
            this.bufferWrite = (this.bufferWrite + 1) % this.BUFFER_SIZE;
        }
        this.bufferAvailable += writable;

        //Check if we have sync
        if (this.bufferAvailable > (this.SAMPLE_RATE * this.CHANNELS * this.LATENCY) && !this.sync) {
            this.Log("Established synchronization.");
            this.sync = true;
        }
    }

    Log(msg) {
        console.log("[OpusAudioWorker] " + msg);
    }

    ProcessOpusFrame(opus) {
        //Decode
        var buf = this.decoder.DecodeFloat(opus);

        //Send
        this.AddBufferSamples(buf);
    }

    OnCmd(data) {
        switch (data.op) {
            case "INIT": this.OnInitCmd(data.payload); break;
            case "FRAME": this.OnFrameCmd(data.payload); break;
            case "VOLUME": this.volume = data.payload; break;
        }
    }
    ...
    OnInitCmd(data) {
        ...
        this.CHANNELS = 2;
        this.BUFFER_SIZE_SECONDS = 2;
        this.LATENCY = 0.25;
        this.BUFFER_SIZE = this.SAMPLE_RATE * this.CHANNELS * this.BUFFER_SIZE_SECONDS;
        this.FRAME_SIZE = (this.SAMPLE_RATE * this.FRAME_DURATION) / 1000;
        ...
        this.sync = false;
        this.opusBuffer = new Float32Array(this.CHANNELS * this.FRAME_SIZE);

        //Subscribe to event
        Module.onload = () => this.OnOpusReady();

        //Load the WebAssembly data
        WebAssembly.instantiate(data.wasm, Module.workerInstantiateInfo).then((r) => {
            Module.workerInstantiateCallback(r.instance);
        });
    }

    OnFrameCmd(data) {
        if (this.decoder != null) {
            this.ProcessOpusFrame(data);
        }
    }

    OnOpusReady() {
        //OPUS is ready to be used. Create OPUS decoder
        //channels, samplerate, maxPacketLen, maxPcmLen
        this.decoder = new Module.Decoder(2, this.SAMPLE_RATE, 65536, this.FRAME_SIZE);
    }

}

registerProcessor('OpusProcessor', OpusProcessor);